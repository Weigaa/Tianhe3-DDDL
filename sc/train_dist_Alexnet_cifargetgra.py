...
def average_gradients(model):
    ...
    for name,param in model.named_parameters():
        if i == 1:
            print("rank ", dist.get_rank(),"number ",i, "name is",name,"begin param.grad.data is", param.tolist())
        dist.all_reduce(param.grad.data, op=dist.reduce_op.SUM)
        param.grad.data /= size
        if i == 1:
            print("rank ", dist.get_rank(), "number ",i, "name is",name,"after param.grad.data is", param.tolist())
        i += 1
    print("total exchange param num is ",i)
